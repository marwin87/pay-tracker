...
| 4 | ~~An existing test passes on SQLite in-memory but the same assertion fails on PostgreSQL due to constraint, type-coercion, or date-arithmetic divergence~~ — **Moot as of the 2026-07-24 Postgres→SQLite migration**: dev, test, and prod all run SQLite now, so there is no second dialect left to diverge from. | High | Medium | Interview Q2; `backend/tests/` test base uses SQLite StaticPool |
...
| 2 | PostgreSQL integration baseline (superseded 2026-07-24) | Replace SQLite fixture with real PostgreSQL; add per-endpoint IDOR integration tests. Reverted by the Postgres→SQLite migration — the app now runs SQLite in prod, so the fixture moved back to SQLite; the IDOR integration tests from this phase remain in place. | #3, #4 | integration | done | context/archive/2026-06-17-testing-postgresql-integration/ |
...
| Backend DB fixture | SQLite StaticPool | — | Current and permanent — app itself runs SQLite in prod as of the 2026-07-24 Postgres→SQLite migration; Phase 2's PostgreSQL fixture was reverted |
...
| pre-prod smoke (manual) | between merge + prod | recommended | environment-specific failures (SMTP, SQLite prod config) |
...
TBD — see §3 Phase 2. Pattern will cover: SQLite fixture setup
(app's own production dialect as of the 2026-07-24 Postgres→SQLite
migration), two-user IDOR test shape, asserting both the 403 response
and the absence of the mutation side-effect.
